<?php
session_start();
require_once __DIR__ . '/../includes/admin_init.php';
$page_title = 'Nova Pergunta';
$conteudos = $conn->query("SELECT c.id, c.titulo, d.nome AS disc FROM conteudo c JOIN disciplina d ON d.id=c.disciplina_id ORDER BY d.nome,c.titulo")->fetch_all(MYSQLI_ASSOC);
$pre_cont = (int)($_GET['conteudo_id'] ?? 0);
$erro = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $cont_id  = (int)($_POST['conteudo_id'] ?? 0);
    $enunc    = trim($_POST['enunciado'] ?? '');
    $ordem    = (int)($_POST['ordem'] ?? 1);
    if (!$cont_id || !$enunc) { $erro = 'Conteúdo e enunciado são obrigatórios.'; }
    else {
        $stmt = $conn->prepare("INSERT INTO pergunta (conteudo_id, enunciado, ordem) VALUES (?,?,?)");
        $stmt->bind_param("isi", $cont_id, $enunc, $ordem);
        if ($stmt->execute()) {
            $perg_id = $conn->insert_id;
            flash('Pergunta criada! Adicione as alternativas agora.');
            header("Location: ../alternativa/criar.php?pergunta_id={$perg_id}"); exit;
        } else { $erro = 'Erro ao salvar.'; }
    }
}
require_once __DIR__ . '/../includes/admin_header.php';
?>
<div class="page-header"><h2>Nova Pergunta</h2><a href="listar.php" class="btn btn-ghost">← Voltar</a></div>
<?php if ($erro): ?><div class="alert alert-error"><?= h($erro) ?></div><?php endif; ?>
<div class="form-card">
  <form method="POST">
    <div class="form-group">
      <label>Conteúdo <span style="color:var(--red)">*</span></label>
      <select name="conteudo_id" required>
        <option value="">Selecione...</option>
        <?php foreach ($conteudos as $c): ?>
          <option value="<?= $c['id'] ?>" <?= ($pre_cont==$c['id']||($_POST['conteudo_id']??'')==$c['id'])?'selected':'' ?>>
            <?= h($c['disc']) ?> › <?= h($c['titulo']) ?>
          </option>
        <?php endforeach; ?>
      </select>
    </div>
    <div class="form-group">
      <label>Enunciado <span style="color:var(--red)">*</span></label>
      <textarea name="enunciado" required placeholder="Digite a pergunta..."><?= h($_POST['enunciado'] ?? '') ?></textarea>
    </div>
    <div class="form-group">
      <label>Ordem</label>
      <input type="number" name="ordem" min="1" value="<?= h($_POST['ordem'] ?? 1) ?>"/>
    </div>
    <div class="form-actions">
      <button type="submit" class="btn btn-primary">Salvar e Adicionar Alternativas →</button>
      <a href="listar.php" class="btn btn-ghost">Cancelar</a>
    </div>
  </form>
</div>
<?php require_once __DIR__ . '/../includes/admin_footer.php'; ?>
